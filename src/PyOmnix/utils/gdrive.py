# Reference: https://github.com/fsspec/gdrive-fsspec/blob/master/gdrive_fsspec/core.py
...
class GoogleDriveFileSystem(AbstractFileSystem):
    ...
    def _list_directory_by_id(self, file_id, trashed=False, path_prefix=None, drive=None):
        all_files = []
        page_token = None
    ...
    def path_to_file_id(self, path, parent=None, trashed=False, drive=None):
        ...
        if len(items) == 1:
            ...
            return self.path_to_file_id(sub_path, parent=top_file_id, trashed=trashed, drive=drive)

    def _get_directory_child_by_name(
        self, child_name, directory_file_id, trashed=False, drive=None
    ):
        all_children = self._list_directory_by_id(directory_file_id, trashed=trashed, drive=drive)
        possible_children = []
