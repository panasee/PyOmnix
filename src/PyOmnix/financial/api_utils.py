...
from datetime import UTC, datetime  # Keep needed datetime objects
...
from .agents.serialization import serialize_agent_state  # Keep
...
from .backend.schemas import (
    AgentExecutionLog,  # Keep
    LLMInteractionLog,  # Keep
)
...
try:
    ...
    from .llm_interaction_logger import set_global_log_storage  # Keep
...
if _has_log_system:
    try:
        ...
        # logger 此时必定已定义
        logger.warning(f"设置全局日志存储器失败: {e!s}")
    ...
    if isinstance(state, str) and len(state) > 0:
        ...
        def direct_logger(request_data, response_data):
            ...
            try:
                ...
                logger.warning(f"保存直接调用的LLM交互到日志存储失败: {log_err!s}")
            ...
            if agent_name:
                ...
                try:
                    ...
                    if _has_log_system:
                        ...
                        log_storage.add_log(log_entry)
                        logger.debug(
                            f"已将装饰器捕获的LLM交互保存到日志存储: {agent_name}"
                        )
                except Exception as log_err:
                    logger.warning(
                        f"保存装饰器捕获的LLM交互到日志存储失败: {log_err!s}"
                    )
        ...
        def wrapper(state):
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"保存Agent执行日志到存储失败: {agent_name}, {log_err!s}"
                    )
                # -----------------------------------------
                ...
                try:
                    ...
                    logger.error(
                        f"保存Agent错误日志到存储失败: {agent_name}, {log_err!s}"
                    )
                # --------------------------------------
