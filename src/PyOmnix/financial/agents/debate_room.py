...
def debate_room_agent(state: AgentState):
    ...
    for msg in state["messages"]:
        ...
        if not hasattr(msg, "name") or msg.name is None:
            continue
        if (
            isinstance(msg.name, str)
            and msg.name.startswith("researcher_")
            and msg.name.endswith("_agent")
        ):
            researcher_messages[msg.name] = msg
            logger.info(f"收集到研究员信息: {msg.name}")
    ...
    if (
        "researcher_bull_agent" not in researcher_data
        or "researcher_bear_agent" not in researcher_data
    ):
    ...
    for name, data in researcher_data.items():
        perspective = data.get(
            "perspective", name.replace("researcher_", "").replace("_agent", "")
        )
