...
from ..api_utils import agent_endpoint, log_llm_interaction
...
def portfolio_management_agent(state: AgentState):
    ...
    # 记录LLM请求
    request_data = {"system_message": system_message, "user_message": user_message}
    ...
    log_llm_interaction(state)(lambda: result)()
...
def format_decision(
    action: str, quantity: int, confidence: float, agent_signals: list, reasoning: str
) -> dict:
    ...
    fundamental_signal = next(
        (
            ...
            if signal["agent_name"] == "fundamental_analysis"
        ),
        None,
    )
    valuation_signal = next(
        (
            signal
            for signal in agent_signals
            if signal["agent_name"] == "valuation_analysis"
        ),
        None,
    )
    technical_signal = next(
        (
            signal
            for signal in agent_signals
            if signal["agent_name"] == "technical_analysis"
        ),
        None,
    )
    sentiment_signal = next(
        (
            signal
            for signal in agent_signals
            if signal["agent_name"] == "sentiment_analysis"
        ),
        None,
    )
    risk_signal = next(
