...
from .. import get_financial_metrics, get_market_cap, search_line_items
from ..data_models import GeneralAnalysis
from ..utils.llm import call_llm
from ..utils.progress import progress
from .state import AgentState, show_agent_reasoning

BenGrahamSignal = GeneralAnalysis
...
def ben_graham_agent(state: AgentState):
    ...
    message = HumanMessage(content=f"Ben Graham Agent: {json.dumps(graham_analysis)}", name="ben_graham_agent")
    ...
    return state | {"messages": [message], "data": state["data"]}
...
def generate_graham_output(
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                f"""Always respond in {language}. You are a Benjamin Graham AI agent, making investment decisions using his principles:
