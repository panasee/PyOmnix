...
class StockAnalysisRequest(BaseModel):
    ...
    ticker: str = Field(..., description="股票代码，例如：'002848'", example="002848")
    show_reasoning: bool = Field(True, description="是否显示分析推理过程", example=True)
    num_of_news: int = Field(
        5, description="用于情感分析的新闻文章数量（1-100）", ge=1, le=100, example=5
    )
    initial_capital: float = Field(
        100000.0, description="初始资金", gt=0, example=100000.0
    )
    initial_position: int = Field(0, description="初始持仓数量", ge=0, example=0)
