...
from ..models.api_models import ApiResponse, StockAnalysisRequest, StockAnalysisResponse
...
async def start_stock_analysis(request: StockAnalysisRequest):
    ...
    future = api_state._executor.submit(
        execute_stock_analysis, request=request, run_id=run_id
    )
    ...
    # 使用ApiResponse包装返回
    return ApiResponse(success=True, message="分析任务已成功启动", data=response)
...
async def get_analysis_status(run_id: str):
    ...
    if not run_info:
        return ApiResponse(
            success=False, message=f"分析任务 '{run_id}' 不存在", data=None
        )
    ...
    try:
        ...
        for agent_name in run_info.agents:
            ...
            if (
                ...
                and agent_data
                and "output_state" in agent_data
            ):
                try:
        ...
        logger.error(f"获取分析结果时出错: {e!s}")
        return ApiResponse(
            success=False,
            message=f"获取分析结果时出错: {e!s}",
            data={"error": str(e)},
        )
