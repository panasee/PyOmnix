...
from ..models.api_models import AgentInfo, ApiResponse
...
async def get_agent_info(agent_name: str):
    ...
    if not info:
        return ApiResponse(
            success=False, message=f"Agent '{agent_name}' 不存在", data=None
        )
...
async def get_reasoning(agent_name: str):
    ...
    try:
        ...
        if not isinstance(serialized_data, dict):
            # 如果不是字典，包装为字典返回
            return ApiResponse(data={"content": serialized_data, "type": "raw_content"})

        return ApiResponse(data=serialized_data)
    except Exception as e:
        # 记录错误并返回友好的错误信息
        logger.error(f"序列化{agent_name}的推理数据时出错: {e!s}")
        return ApiResponse(
            success=False,
            message=f"无法处理{agent_name}的推理数据: {e!s}",
...
async def get_latest_llm_request(agent_name: str):
    ...
    try:
        ...
        if not isinstance(serialized_data, dict):
            # 如果不是字典，包装为字典返回
            serialized_data = {"content": serialized_data, "type": "raw_content"}

        return ApiResponse(data=serialized_data)
    except Exception as e:
        logger.error(f"处理{agent_name}的LLM请求数据时出错: {e!s}")
        return ApiResponse(
            success=False,
            message=f"无法处理{agent_name}的LLM请求数据: {e!s}",
            data={"error": str(e)},
        )
...
async def get_latest_llm_response(agent_name: str):
    ...
    try:
        ...
        logger.error(f"处理{agent_name}的LLM响应数据时出错: {e!s}")
        return ApiResponse(
            success=False,
            message=f"无法处理{agent_name}的LLM响应数据: {e!s}",
            data={"error": str(e)},
        )
