...
class SimplifiedBacktester:
    ...
    def analyze_performance(self, price_data):
        ...
        for date, price in price_data.iterrows():
            total_value = (
                self.portfolio["cash"] + self.portfolio["stock"] * price["close"]
            )

            # 计算日收益率
            if portfolio_values:
                daily_return = (
                    total_value / portfolio_values[-1]["Portfolio Value"] - 1
                ) * 100
            else:
                daily_return = 0
        ...
        performance_df["Cumulative Return"] = (
            performance_df["Portfolio Value"] / self.initial_capital - 1
        ) * 100
    ...
    def _plot_performance(self, performance_df):
        ...
        line1 = ax1.plot(
            performance_df.index, portfolio_values_k, label="组合价值", marker="o"
        )
        ...
        # 在数据点上添加标签
        for x, y in zip(performance_df.index, portfolio_values_k, strict=False):
            ax1.annotate(
                f"{y:.1f}K",
                (x, y),
        ...
        # 在数据点上添加标签
        for x, y in zip(performance_df.index, performance_df["Cumulative Return"], strict=False):
            ax2.annotate(
                f"{y:.2f}%",
                (x, y),
                textcoords="offset points",
...
def test_backtest():
    ...
    # 执行交易
    for (date, price), (action, quantity) in zip(price_data.iterrows(), decisions, strict=False):
        executed_quantity = backtester.execute_trade(action, quantity, price["close"])
        print(
            f"日期: {date.strftime('%Y-%m-%d')}, 价格: {price['close']:.2f}, "
